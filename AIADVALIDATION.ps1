...
$odlId = "7138"  ## Replace with the ODL Id of the lab
$stopsqlpooloncesuccessfull = "No"  ## Allowed values "Yes" or "No"
$stopVMsoncesuccessfull = "No"  ## Allowed values "Yes" or "No"
 ...
 foreach($rg in $rgs)
{
   ...
   $global:userName="replaceusername@msazurelabs.onmicrosoft.com"  #replaceusername
   $global:password="replacepassword"    #replacepassword
...
#Overallstate  
    ...
    if (($overallStateIsValid -eq "true") -and ($overallpipelinestatus -eq "Success")  ) {
        ...
        if ($stopVMsoncesuccessfull -eq "Yes"){
        ##Stopping VMs
        Stop-AzVM -ResourceGroupName $vmrgname -Name $vmName -Force
